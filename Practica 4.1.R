...
# 1.- Creación de un vector de color
color=c("darkorchid1", "gold", "cyan")

# 2.- Creación del gráfico
GB1<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "chocolate", fill=color)+
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="deeppink", fill="gold2")+
  ggtitle("Histograma")+
...
GD2<-ggplot(penguins, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  ...
  scale_colour_manual(values=c("gold","cyan","deeppink"))+
